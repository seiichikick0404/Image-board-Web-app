    ...
    <?php foreach ($replies as $reply): ?>
        <div class="reply-container">
            ...
            <div class="tweet-header d-flex align-items-center justify-content-between">
                <!-- 返信のタイトル表示部分を削除し、日付の表示を返信のデータに基づいて更新 -->
                <p class="post-date"><?php echo(htmlspecialchars($reply->getTimestamp()->getCreatedAt())); ?></p>
            </div>
            <p class="post-content"><?php echo(htmlspecialchars($reply->getContent())); ?></p>
            <?php if (!empty($reply->getImagePath())): ?>
                <a href="../../public/storage/<?php echo(htmlspecialchars($reply->getImagePath())); ?>" target="_blank">
                    <img src="../../public/storage/<?php echo(htmlspecialchars($reply->getImagePath())); ?>" alt="返信画像" class="post-image">
                </a>
            <?php endif; ?>
        </div>
    <?php endforeach; ?>
